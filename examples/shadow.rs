...
use std::{collections::HashMap, thread::{Thread, self}};
...
pub fn main() {
    ...
    let host1 = Root::new(Host{processes: HashMap::new()});
    {
        ...
        host1_lock.processes.insert(0, Process{ descriptors: HashMap::new() });
        host1_lock.processes.get_mut(&0).unwrap().descriptors.insert(0, descriptor.clone());

        // So does Process 1.
        host1_lock.processes.insert(1, Process{ descriptors: HashMap::new() });
        host1_lock.processes.get_mut(&1).unwrap().descriptors.insert(0, descriptor.clone());
    }
    ...
    let worker = thread::spawn(move ||  {
        for (host_id, host) in &hosts {
            ...
            lock.processes.get_mut(&0).unwrap().descriptors.insert(2, descriptor);

            // Iterate
            for (pid, process) in &lock.processes {
                for (fid, descriptor) in &process.descriptors {
                    println!("host_id:{} pid:{} fid:{} open:{}", host_id, pid, fid, descriptor.open);
                }
            }
        }
        hosts
